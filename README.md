...
当前构建：**v1.3.0**（对话自由编辑、断点续写、单角色卡导入导出、接口测速诊断与沉浸交互全面优化）
...
本次版本重点针对跑团与长文创作中的高频交互痛点进行了**人性化深度改造与沉浸体验升级**：

- **消息全自由编辑（Edit Message）**：遇到 AI 回复细节微瑕或用户自己输入笔误时，长按消息或点击动作栏即可直接原地修正文本并持久化，省去重写整轮的烦恼。
- **断点无缝续写（Continue）**：针对超长小说剧情输出或被模型 max tokens 截断的回复，助手气泡下方新增「续写」按钮，无缝承接前文继续流式生成。
- **单角色卡独立导入与导出（JSON）**：角色编辑页支持一键将角色卡序列化为 JSON 导出并复制至剪贴板；主页角色栏支持快速粘贴导入，方便模组分享与快速导入。
- **API 连通性测速与诊断（Ping & Test）**：API 配置页面新增「测试连接」按钮，毫秒级实测网络延迟，并在密钥错误、Endpoint 路径偏差或网络超时时给出直观清晰的诊断指引。
- **长文本局部自由选词（Text Selection）**：AI 消息正文全面支持系统原生自由文本选择与长按选段复制，随时摘录精彩金句。
- **一键回到底部悬浮球（Scroll to Bottom）**：向上翻阅漫长历史对话时，右下角自动浮现「↓」快捷按钮，轻点即刻丝滑回滚至最新输入气泡。
