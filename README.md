# 暮色酒馆 Project Tavern

本地优先的 Android AI 角色 / RPG 客户端。**你自带模型，酒馆只负责世界。**

> Bring your own AI. Bring your own world.

App **不提供推理、不做 API 中转、没有账号、没有云同步**。Key 只存在这台手机上。

当前构建：**v1.5.0**（数据原子安全保障、流式60ms节流与智能跟随、草稿引用隔离、{{char}}宏与O(N)上下文裁剪、DeepSeek-R1思维链兼容）

> [!NOTE]
> **版本与源码说明**：
> - **开源代码库（Community Lite Edition）**：本仓库提供核心客户端骨架、Android UI 交互与标准模型通信能力，适合二次开发与定制。
> - **官方正式安装包（Official Packaged Release）**：包含完整高级多级世界书注入引擎、记忆浓缩流与官方预设剧本。推荐直接前往 [GitHub Releases](https://github.com/Aliang-ye/ProjectTavern/releases) 下载官方构建的 APK 安装包体验完整功能。

## 版本说明 (v1.5.0)

本次更新聚焦于“数据安全护航、高速流式流畅度跃迁、编辑引用完全隔离、Prompt 引擎规范化与全场景体验打磨”：

1. **底层数据安全与防丢机制强化**：
   - **异常损坏自动备份**：当 `tavern.json` 解析异常时，自动备份损坏源文件为 `tavern.json.bak.<timestamp>`，彻底告别以往解析失败直接静默清空用户数据的风险；
   - **原子写入机制**：写盘采用临时文件写入后再进行原子重命名，防止写入中途进程中断或断电导致损坏；
   - **完全脱离 UI 线程**：将 JSON 序列化与写盘动作全部移入后台专用 IO 线程，无论对话量多大均不阻塞前台操作。
2. **高速流式输出 60ms 节流与智能跟随**：
   - **60ms 批次节流刷新**：针对 DeepSeek、Groq 等高速输出模型（50–100 tokens/s），聚合 token 并在 UI 线程以 60ms 间隔平滑渲染，彻底消灭高速打字时的严重卡死与掉帧；
   - **智能自动滚动判定**：仅当用户处于消息底部时自动跟随滚动；用户向上滑动翻阅历史上下文时，绝不强行将视图拉回底部。
3. **编辑草稿完全隔离与取消保护**：
   - 修复在角色、世界书、故事编辑页面中直接引用 live 对象的底层 Bug，全面采用副本隔离机制；用户若未点击保存直接退出，原数据丝毫不受影响。
4. **Prompt 引擎与上下文优化**：
   - **`{{char}}` 宏全面支持**：解决从各类社区导入角色卡时 `{{char}}` 无法正确替换为角色/世界意志名称的问题；
   - **O(N) 上下文裁剪算法**：优化上下文超长裁剪逻辑，从尾部按字符长度计数截断，杜绝长会话频繁 joinToString 造成的性能损耗。
5. **网络鲁棒性与 DeepSeek-R1 思维链支持**：
   - **90 秒兜底读取超时**：防止网络静默断开导致请求无限挂起无响应；
   - **DeepSeek-R1 思维链捕获**：当模型返回 `reasoning_content` 时自动捕获并无缝呈现；
   - **SSE 流容错**：对格式异常的数据行自动跳过，保障长文本流式生成的完整交付。
6. **人性化操作与交互打磨**：
   - **用户消息操作直观化**：用户发出的消息气泡下方直接提供编辑、复制与删除入口，与助手消息保持一致设计语言；
   - **导入备份防误触确认**：从本地恢复备份前弹出防误触警告确认窗；
   - **生成模式体验升级**：改为失去焦点与切出应用时自动持久化，不再逐键写盘；增加数值范围智能校验。

Android APK 发布与下载：

- **最新版本 Release**：[Project Tavern v1.5.0](https://github.com/Aliang-ye/ProjectTavern/releases/tag/v1.5.0)
- **最新安装包直接下载**：[ProjectTavern-v1.5.0.apk](https://github.com/Aliang-ye/ProjectTavern/releases/download/v1.5.0/ProjectTavern-v1.5.0.apk)
- **所有历史版本**：https://github.com/Aliang-ye/ProjectTavern/releases

## 能做什么

| 页 | 说明 |
| --- | --- |
| **角色** | 创建 / 编辑 / 复制 / 删除。描述、性格、场景、开场白、示例对话、系统提示。可直接「单独对话」。 |
| **世界书** | 概况、地理、历史、制度、人文、个人补充设定（始终注入）。另有高级条目测试。 |
| **故事** | 长期 RPG 存档。主角 / 同伴 / 路人 + 故事世界书。 |
| **对话** | 流式输出、停止、编辑、删除、重写、多版本 Generation。小说阅读感。 |
| **设置** | 主题、语言、用户主体、API、生成模式、流式/自动摘要、开发者模式、备份。 |

## 原则

1. AI 是用户自己的，Tavern 是控制 AI 的工具。
2. Character、World、Story、Memory、Chat 解耦。
3. 本地优先。App 不依赖自有后端。
4. 只保留纯工具型体验，不引入社区、商城、账号或云同步。

## API（BYOK）

只需两种格式：

| 类型 | 适用 |
| --- | --- |
| **OpenAI 兼容** | OpenAI、DeepSeek、xAI、Groq，以及任何 `/v1/chat/completions` |
| **本地 / 局域网模型** | Ollama（`http://192.168.x.x:11434/v1`）、LM Studio、SillyTavern |
| **Claude** | Anthropic 官方或兼容 Messages 接口 |

Key 仅保存在本机 `filesDir/tavern.json`，**导出备份时会自动去掉 Key**。

浏览器预览可能被跨域拦住；**Android 客户端直连，无此限制**。

## 语言与人格

两套纯语言，不混合：

- 界面中文 → 模型只写简体中文
- 界面 English → 模型只写英文

设置里填写你的名字和人设，会替换 `{{user}}`，并作为 `PERSONA` 块注入每一轮提示词。

## 生成模式

内置四套（中/英各一份），对话页可随时切换：

| 模式 | 用途 |
| --- | --- |
| 小说模式 | 第三人称有限视角，动作 / 光线 / 气味 |
| 创意模式 | 意象更密，仍保持角色声音 |
| 快速模式 | 短句、多对话 |
| 推理模式 | 先核对设定再写正文，不把思考过程写出来 |

## 主题

深色烛火 / 浅色原木纸张 / 跟随系统。

## 使用方式

1. 在设置里创建一个 API 配置
2. 选择 OpenAI 兼容或 Claude
3. 填写模型、接口地址、API Key
4. 在角色页或故事页建立对话
5. 进行角色扮演、世界书注入和长对话创作

首次会带示例角色「艾莉丝 / 莱恩」和世界书「暮色酒馆」。

## 环境

- Android 8.0（API 26）+
- 编译：JDK 17、Android SDK 34、Gradle 8.7（已带 Wrapper）
- 权限：仅 `INTERNET`

## 用 Android Studio 打开

1. `File → Open` 选本仓库根目录
2. 等待 Gradle 同步
3. 连真机或模拟器 Run

## 命令行打 Debug 包

```bash
# 根目录创建 local.properties（不要提交）
# sdk.dir=/your/Android/Sdk

chmod +x gradlew
./gradlew assembleDebug
```

产物：`app/build/outputs/apk/debug/app-debug.apk`

debug 签名，仅供自己安装测试。换机器重新打包可能要先卸载再装。

## 数据存在哪

`filesDir/tavern.json`。导出备份不含 API Key。恢复备份时会保留当前 Key。

## 明确不做

公共社区、账号系统、云同步、自有模型、API Proxy、商城、广告。

## 许可

MIT。见 [LICENSE](LICENSE)。
