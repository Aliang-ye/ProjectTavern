...
当前构建：**v1.6.0**（真实用户体验打磨、最后消息内容直观预览、空列表友好引导、对话长按自定义重命名、一键错误重试、发送自动收键盘、角色卡长按导出JSON、续写平滑节流、按需轻量渲染）
...
本次更新站在真实玩家与高频使用者的第一视角，解决日常使用中最直接的视觉与交互痛点：

1. **对话列表最后消息预览**：
   - 会话卡片不再只显示生硬标题，副标题实时呈现最近一条消息的前 60 字剧情预览，海量对话一眼辨识，无需反复逐一点击查看。
2. **空状态友好引导**：
   - 角色、世界书、故事、对话等模块为空时增加温馨引导文案，告别以往一无所有的白屏茫然感，新手第一眼即知如何开始。
3. **对话自定义长按重命名**：
   - 对话顶部标题支持长按弹出重命名对话框，玩家可随时随地给心仪的剧情分支命名（如“第七章·雪夜长谈”），告别系统千篇一律的自动起名。
4. **生成报错一键重试**：
   - 网络波动或 API 报错时，错误行旁常驻“↺ 重试”快捷按钮，一键重新生成，彻底告别被迫重新手打上一句行动的糟糕体验。
5. **发送消息自动收起键盘**：
   - 点击发送后软键盘立即平滑收起，聊天正文与流式生成过程全屏开阔展示，不再被输入法遮挡视线。
6. **角色卡一键导出 JSON**：
   - 角色编辑页面长按“保存”按钮即可快速打包导出角色卡 JSON 并唤起系统分享，方便与好友或社区交流角色设定。
7. **“继续”续写流式节流与按需轻量渲染**：
   - 续写（Continue）逻辑全面同步 60ms 节流批处理机制，杜绝断点续写时的高速掉帧；主界面底部 Tab 切换改为当前面板按需刷新，提升日常操作流畅度。

Android APK 发布与下载：

- **最新版本 Release**：[Project Tavern v1.6.0](https://github.com/Aliang-ye/ProjectTavern/releases/tag/v1.6.0)
- **最新安装包直接下载**：[ProjectTavern-v1.6.0.apk](https://github.com/Aliang-ye/ProjectTavern/releases/download/v1.6.0/ProjectTavern-v1.6.0.apk)
